...
namespace NamelessBot.Bot.Services {
    public class MessagebookService {
        ...
        private async Task _socketClient_MessageButtonClicked(string value, SocketUser user, IMessage message, SocketTextChannel channel, SocketGuild guild) {
            var data = JsonConvert.DeserializeObject<ReviewAction>(value);
            if (data.Type == "messagebook_review") {
                await ReviewMessage(data.Id, data.MessageId, data.Pass, message.Id);
            }
        }
        ...
        public async Task CreateMessagebook(string title, string description, ulong channelId) {
            ...
            var book = new Messagebook { Title = title, Description = description, ChannelId = channelId, GuildId = channel.Guild.Id };
            var cards = new MessagebookCard(book, _socketClient.CurrentUser.Id).Build();
            await channel.SendCardMessageAsync(cards[0]);
            await channel.SendKMarkdownMessageAsync($"`.mb message {book.Id} 留言内容`");
            book.MessageId = (await channel.SendCardMessageAsync(cards[1])).MessageId;
        ...
        public async Task RemoveMessagebook(Guid id) {
            if (_messagebooks.Find(m => m.Id == id) is Messagebook messagebook) {
                ...
                await File.WriteAllTextAsync($"messagebooks/{messagebook.Title}-{DateTimeOffset.Now.ToString("yyyy-MM-dd-HH-mm-ss-ms")}.json", JsonConvert.SerializeObject(messagebook, Formatting.Indented));

                SaveMessagebooks();
                await (await _socketClient.GetChannelAsync(messagebook.ChannelId) as SocketTextChannel).SendKMarkdownMessageAsync("你现在可以安全地删除留言板消息了");
        ...
        public async Task UpdateMessagebook(Guid id) {
            if (_messagebooks.Find(book => book.Id == id) is Messagebook messagebook) {
                ...
                await (await _socketClient.GetChannelAsync(messagebook.ChannelId) as SocketTextChannel).ModifyMessageAsync(messagebook.MessageId, (properties) => {
        ...
        public async Task CreateMessage(Guid id, string message, IUser user) {
            if (_messagebooks.Find(book => book.Id == id) is Messagebook messagebook) {
                ...
                SaveMessagebooks();
                _logger.LogInformation("已在留言板 {0}({1}) 创建 {2}({3}) 的留言: ({4}){5}", messagebook.Title, messagebook.Id, $"{user.Username}#{user.IdentifyNumber}", user.Id, messagebookMessage.Id, messagebookMessage.Message);
        ...
        public async Task SendReviewMessage(Messagebook messagebook, MessagebookMessage message) {
            if (messagebook.ReviewChannelId.HasValue && !message.IsReview) {
                _logger.LogInformation("发送审核消息留言板 {0}({1}) {2} 的留言: ({4}){5}", messagebook.Title, messagebook.Id, message.CreatorId, message.Id, message.Message);
                await (await _socketClient.GetChannelAsync(messagebook.ReviewChannelId.GetValueOrDefault()) as SocketTextChannel).SendCardMessageAsync(new MessagebookMessageReviewCard(messagebook, message).Build());
            }
        }
            ...
            if (_messagebooks.Find(book => book.Id == id) is Messagebook messagebook) {
                if (messagebook.Messages.Find(m => m.Id == messagebookMessageId) is MessagebookMessage message) {
                    ...
                    if (pass) {
                        ...
                        await (await _socketClient.GetChannelAsync(messagebook.ReviewChannelId.GetValueOrDefault()) as SocketTextChannel).ModifyMessageAsync(reviewMessageId, (properties) => {
                            properties.Cards = new MessagebookMessageReviewResultCard(messagebook, message).Build().ToList();
                        });

                        _logger.LogInformation("已通过 {0}({1}) 的在留言板 {2}({3}) 的消息: ({4})({5})", $"{ownerUser.Username}#{ownerUser.IdentifyNumber}", message.CreatorId, messagebook.Title, messagebook.Id, message.Id, message.Message);
                    } else {
                        messagebook.Messages.Remove(message);
                        await (await _socketClient.GetChannelAsync(messagebook.ReviewChannelId.GetValueOrDefault()) as SocketTextChannel).DeleteMessageAsync(reviewMessageId);
                        _logger.LogInformation("审核未通过，删除 {0}({1}) 的在留言板 {2}({3}) 的消息: ({4})({5})", $"{ownerUser.Username}#{ownerUser.IdentifyNumber}", message.CreatorId, messagebook.Title, messagebook.Id, message.Id, message.Message);
                    }
        ...
        public async Task SetReviewChannel(Guid id, SocketTextChannel channel) {
            if (_messagebooks.Find(book => book.Id == id) is Messagebook messagebook) {
                ...
                SaveMessagebooks();
                _logger.LogInformation("频道 {0}({1}) 已被设为 {2}({3}) 的审核频道", channel.Name, channel.Id, messagebook.Title, messagebook.Id);
                await channel.SendKMarkdownMessageAsync($"该频道已被设为 {messagebook.Title} ({messagebook.Id}) 的审核频道");
